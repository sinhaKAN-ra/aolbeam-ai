...
const GeneratePracticeProblemOutputSchema = z.object({
  problemType: z.enum(['theory', 'practical', 'practical_mcq', 'conceptual', 'numerical', 'diagram_based']).describe('The type of problem that was generated.'),
  ...
  multipleChoiceOptions: z.array(z.string().describe(`Multiple choice option. Should use LaTeX for math, Markdown for code, and describe/use Mermaid for diagrams if applicable.`)).describe(`Multiple choice options. Required for "practical_mcq" type and can be used for "conceptual" or "numerical" if in MCQ format. Must be an empty array for "theory", "practical", and "diagram_based" types.`),
...
const prompt = ai.definePrompt({
  ...
  output: {schema: GeneratePracticeProblemOutputSchema.omit({ difficulty: true, problemType: true })},
...
- problemStatement: Must pose a clear calculation or application question
- multipleChoiceOptions: MUST be an empty array []
- correctAnswer: Must be the numerical result or step-by-step solution
- answerFormat: Must provide guidance on how to approach the solution

**FOR "practical_mcq" PROBLEMS:**
...
- If MCQ format: multipleChoiceOptions must contain 4 options, correctAnswer must match exactly
- If theory format: multipleChoiceOptions must be [], correctAnswer must be a complete model answer
...
- multipleChoiceOptions: MUST be an empty array []
- correctAnswer: Must be a complete explanation or analysis of the diagram
- answerFormat: Must specify expected answer structure
...
Generate your response now following ALL requirements above. Remember:
- For 'practical_mcq': always include exactly 4 multipleChoiceOptions and ensure correctAnswer matches one exactly
- For 'practical': multipleChoiceOptions must be an empty array []
- For 'diagram_based': multipleChoiceOptions must be an empty array []
- For 'theory': multipleChoiceOptions must be an empty array []
- For 'conceptual'/'numerical': follow the specific format instructions given earlier`, // add a newline here so output starts on a new line
});
